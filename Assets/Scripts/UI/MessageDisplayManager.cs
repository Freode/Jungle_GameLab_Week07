...
public class MessageDisplayManager : MonoBehaviour
{
    ...
    /// <summary>
    /// 화면 중앙 상단에 메시지를 표시합니다. 메시지는 연결된 게임 오브젝트가 파괴될 때까지 표시됩니다.
    /// </summary>
    /// <param name="message">표시할 메시지 텍스트</param>
    /// <param name="color">메시지 색상</param>
    /// <param name="linkedObject">메시지의 생명주기와 연결될 게임 오브젝트</param>
    public void ShowMessageUntilDestroyed(string message, Color color, GameObject linkedObject)
         {
             if (messageText == null) return;
            ...
            currentMessageCoroutine = StartCoroutine(DisplayUntilDestroyedCoroutine(linkedObject));
        }
   
        private IEnumerator DisplayUntilDestroyedCoroutine(GameObject linkedObject)
        {
            // linkedObject가 파괴될 때까지 대기
            // Yield an error if linkedObject is null initially.
            if (linkedObject == null)
            {
                Debug.LogError("Error: linkedObject is null. Cannot track its destruction.");
                yield break;
            }
   
            yield return new WaitUntil(() => linkedObject == null);
